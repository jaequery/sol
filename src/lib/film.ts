 ...
 *
 * The leg is found by the generation's `filmSegmentIndex` and never by arrival order, so leg 2 finishing
...
export function applyGenerationToFilm(film: Film, generation: FilmGeneration): Film {
  const index = generation.target.filmSegmentIndex;
  const segment = film.segments.find((s) => s.index === index);
