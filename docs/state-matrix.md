...
| 6c | **Two ways media gets in** | Any state | Beside + Import the bin head carries **✦ Generate**, which opens the compose panel (section 12). Importing is untouched by it: the two are separate actions on the same panel, and neither is behind the other | Click either |
